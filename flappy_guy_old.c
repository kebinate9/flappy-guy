...
int main(void) {
  ...
  Rectangle flapper = {
    .x = flapper_posx,
    .y = flapper_posy,
    .width = flappers[flapper_index].width,
    .height = flappers[flapper_index].height
  };
  ...
  UpdateMusicStream(soundtrack);
    ...
    BeginDrawing(); // drawing loop
      ...
      if(!is_dead) {
        ...
        printf("x=%d, y=%f fps=%i is_dead=%s\n", flapper.x, flapper.y, GetFPS(), is_dead ? "true" : "false"); // debug line
        if(flapper_posy == 490 || CheckCollisionRecs(flapper, obstacles[1].dest)) {
          is_dead = true;
  ...
  UnloadTexture(flappers[flapper_index]);
  UnloadTexture(flappers[flapper_index]);
